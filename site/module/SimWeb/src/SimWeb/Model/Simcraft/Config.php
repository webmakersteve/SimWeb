...
class Config {
	...
	public function generateText() {
		...
		$return[] = $this->kp( "origin", sprintf("http://simcraft.cloudapp.net/simulate/%s/%s/%s", $this->character->getRegion(true), $this->character->getRealm(true), $this->character->getName(true)) );
		...
		foreach( $this->character->getEquipment() as $piece => $data ) {
			$return[] = $this->kp($piece, sprintf("%s,id=%d,stats=%s,%s%s%s%s",
				$this->n($data->Name()),
				$data->ID(),
				$data->getSimStatsStr(), 
				$data->getSimGemStr(),
	...
	private function kp( $str1, $str2 ) {
		...
		return sprintf( "%s=%s", $str1, rtrim($str2, ",") );	
	}
...
legs=leggings_of_the_horned_nightmare,id=99098,stats=2417armor_2081int_3362sta_1502crit_1267mastery,,
